...
**Open PRs:** #120 document upload/download/remove -- green except the `.env.example` guard (UPLOAD_MAX_BYTES), which only a human can add; `feat/S6-keycloak-auth` -- login, roles, grants, activity log, same guard (AUTH_MODE and the KEYCLOAK_* lines).

**Still human-only, and blocking nothing else:** a named owner for the law 09-08 personal-data check; a hand-run of login against a real Keycloak in Docker; the document-route role guards, which land when #120 merges and the login branch rebases on it.

---

## STATE AT 2026-09-13, S6 v3 IN PROGRESS (history now)
...
- #42 `ee143a3` the `report-brief` skill and a ENGINEERING-RULES.md reporting rule.
...
- #45 `12f4e0c` the core law folded into ENGINEERING-RULES.md, plus the `reviewer`
  ...
  branches and an `[AI]` marker, which ENGINEERING-RULES.md rules 2 and 4 forbid.
- #41 `249724a` this file's own ST-17 review pass.

STILL NOT TRACKED, and it is the one thing keeping the above at documents
rather than enforcement: `.devtools/settings.json`. The two hooks
...
- "54 jours de cotisation appears three times with three different windows
  ...
  **two** distinct windows (6, 6, 10, 6). It had already been copied into the
  README, this file and CHANGELOG.
...
without the schema and tests that existed only there. That breaks ENGINEERING-RULES.md
...
committed: docs/phase2/ENGINEERING-RULES.md forbids keys in tests, fixtures and CI.
...
1. The seam is Sanad's own `complete(system, user) -> str`, NOT
   ...
   project's own rule -- docs/phase2/ENGINEERING-RULES.md requires a scripted fake
...
- AN ANSWER WITH NO SOURCES CANNOT EXIST. The rule is written in three
  signed documents (openapi's G3 note, docs/phase2/ENGINEERING-RULES.md, PRD F-03)
...
docs/phase2/ENGINEERING-RULES.md lines 33-34, PRD F-03 to F-10 and section 11, UX
...
- MERGED WITHOUT THE RULE-5 REVIEW PASS on 2026-08-23, at the human's
  ...
  point: ENGINEERING-RULES.md rule 5 exists because a green suite has never once
...
Rule:       8 of the 20 above name a specific article of the labour code.
            ...
            claimed a verification that had not happened:
              ...
              - 72 certificat de travail, 205 repos hebdomadaire -- both
                ...
                CHANGELOG. These two did not exist when the review ran.
...
because the good fix deviates from a signed spec (ENGINEERING-RULES.md rule 1).
...
files under `.devtools/` are tracked, `settings.json` and its two hooks
...
WHAT IT SAID: "ENGINEERING-RULES.md gives the project name as
...
written into ENGINEERING-RULES.md -- and that broke MB's machine, where
...
THE FIX, and it is not a third name: ENGINEERING-RULES.md no longer names a project
...
about are already gone from ENGINEERING-RULES.md, replaced by "no node count is
...
9. FOUND WHILE DOING 8, and it was already on main: `.env.example` began
   ...
   on the committed bytes first. ENGINEERING-RULES.md's Windows write trap named this
...
- **G4 CANNOT BE MEASURED ON MB's MACHINE. Raised 2026-08-29 by ST-18.**
  ...
  the machine keeps the old value. ENGINEERING-RULES.md's graph project name was the
...
- HALF CLOSED 2026-08-27, and closing it found a defect nothing else
  ...
  (docs/phase2/ENGINEERING-RULES.md rightly forbids API keys in tests, so no test
...
- WAS, and kept because the reasoning is still the record of how it was
  ...
  ST-25: docs/phase2/ENGINEERING-RULES.md already mandates that tests use a scripted
...
- HARNESS GAPS after `chore/harness-fit` (2026-08-22). TWO OF THE FRAMING
  ...
  (i)  "the permissions deny-list blocks `Edit(**/.devtools/hooks/**)`,
       `Edit(**/.devtools/settings.json)` and `Edit(~/.config/devtools/**)`, so no agent
       can close any of them". There is no such deny-list any more. Both
       `~/.config/devtools/settings.json` and `.devtools/settings.local.json` have an
  ...
  1. CLOSED 2026-08-23. The project-level `.devtools/settings.json` this item
     ...
     (`"C:\Users\lenovo\.devtools\hooks\cbm-*.cmd"`), which was tested from
     ...
     dev tooling itself invokes them cleanly at a real SessionStart. The
     next session start is the test.
  2. `~/.config/devtools/hooks/` carries the SAME two defects fixed here in 368e3ea:
  ...
  3. Guard gap, found by tripping it: `config-guard.mjs`'s protected-path
     regex is `/(^|[^\w.])\.devtools([\/\\]|$)/i`, which requires `.devtools` to be
     followed by a slash or end-of-string. A bare `.devtools` argument (as in
     `git rm -r --cached .devtools`) does NOT match and passes. That is how this
  ...
  4. Guard false-positive on READ-ONLY inspection, hit twice this session: any
     ...
     a mention of `.devtools` is blocked. Same shape as the old shell guard's
  ...
  6. CLOSED 2026-08-23 by the ST-17 session, which is the next session start
     ...
     gap; the only exclusions are by design (`.devtools`, `.venv`, `data`,
...
- SAFETY SYSTEM: critical half FIXED 2026-08-01, remainder is low-priority
  ...
  it abandoned work rather than blocked work:
    Owner:    a human. Rule 4 locks agents out of `.devtools/hooks/` entirely,
...
- Open, unowned: data-layer follow-ups 1, 2 and 6 below. Deliberately NOT
  ...
  would have put unreviewable drive-by edits in PR #14 (.devtools/rules
...
- Harness repaired, branch `chore/harness-fit` (2026-08-22). The control
  plane came BACK into the repo at project level (PR #31, `.devtools/` now
  ...
  (a) the Stop gate was a permanent no-op. `gate.mjs` hard-exited on a
      ...
      `.devtools/gate-last-failure.log`; restored, it goes green again;
  (b) `guardedPaths()` was exported by `_config.mjs` and imported by NO
      ...
      ENGINEERING-RULES.md rule 4 calls that lock non-negotiable; it did not exist;
  ...
  `rm -f .devtools/gate-last-failure.log` with exit 2. That was a correct
...
- Harness migration completed. MERGED as 275886f (PR #25). Finished a
  half-done move: the assistant harness installer zip had unpacked into the
  ...
  plane deleted (41 files: .devtools/agents, .devtools/hooks, .devtools/rules,
  .devtools/skills, .devtools/settings.json) since the control plane now
  lives at the user level (`~/.devtools`); ENGINEERING-RULES.md corrected to stop
  ...
  half-finished unpack), moved out to ~/the assistant-setup/ and the by-then-dead
  ignore entries removed on chore/separate-harness-payload -- see that
  branch's CHANGELOG line. Graph re-verified at 769 nodes/2,392 edges
...
- Root README.md with the three-phase Mermaid pipeline diagram (ingestion,
  ...
  (40e4ac0).
- MB's `.devtools/worktrees/` and `.devtools/agent-memory/` gitignore. MERGED,
  PR #6 (c844922).
...
- Workflow rule added: no AI attribution in commits, PRs, or issues - no
  ...
  normally. Written into ENGINEERING-RULES.md rule 6, the team git-discipline rules,
