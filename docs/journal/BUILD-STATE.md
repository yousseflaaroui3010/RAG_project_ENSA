...
- HARNESS GAPS still open after `chore/harness-fit` (2026-08-22). All six are
  HUMAN-ONLY: the permissions deny-list blocks `Edit(**/.devtools/hooks/**)`,
  `Edit(**/.devtools/settings.json)` and `Edit(~/.config/devtools/**)`, so no agent can
  ...
  1. `.devtools/settings.json` still registers the CBM hooks as bare
     extensionless paths (`~/.config/devtools/hooks/cbm-session-reminder`). On Windows
