...
20. **How long to add documents?** (YL) Below target, and we say so:
    731.6 s per 200 pages against a 600 s goal. Most of the measured time
    is the embedding model on a CPU (about 0.29 s per chunk) and PDF
    reading. A second Sync of unchanged files takes 0.13 s.
