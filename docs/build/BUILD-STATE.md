...
- CRITICAL, found 2026-08-01, needs a human because agents are locked out of
  ...
  Layer 1, `.devtools/hooks/guard.sh`: it is correctly registered as a
  ...
  .devtools/hooks + settings.json protection, and the journal-debt gate.
  ...
  Layer 2, `.devtools/git-hooks/pre-commit`: never installed. `.git/hooks/` is
  ...
  human-only:
    1. `cp .devtools/git-hooks/pre-commit .git/hooks/pre-commit && chmod +x
       .git/hooks/pre-commit` -- restores layer 2 (one rule).
    2. In `.devtools/hooks/guard.sh` line 8, make the interpreter resolve:
