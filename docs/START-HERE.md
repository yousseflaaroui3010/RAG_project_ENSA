...
3. **Do the work.** Either write it, or ask **dev tooling** to build it: in the
   dev tooling chat box (in your terminal or the VS Code panel), type plainly,
...
- **See it in the browser (starter screen):** we'll add a tiny opening screen so
  ...
  - ⌨️ `uv run python app.py` → then 🖱️ open **http://127.0.0.1:8000**. (Ask
    dev tooling to add this starter screen — a 5-minute step that makes "the app
    runs!" true today.)
...
- **A command turned red?** Copy the red text exactly and ask dev tooling / your
