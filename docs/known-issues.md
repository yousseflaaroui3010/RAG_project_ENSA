...
| It is unconfirmed whether `railway.json` is applied at all | The health gate it declares may not be in force. On the first deploy carrying the file (`3b56c35`), the `sanad-web` manifest still reported `healthcheckPath None` | Railway's docs say a root config file is picked up automatically and that the deployment details page marks such settings with a file icon. Nobody has opened that page. Until someone does, treat the health gate as declared, not active |
