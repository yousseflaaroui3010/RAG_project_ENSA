...
| The demo's Keycloak is not linked to GitHub and has a redeploy trap | Changing a variable on the Railway `keycloak` service without `--skip-deploys` redeploys the plain published image, which only prints help and stops; a merge to `main` never updates it | It is uploaded with `railway up <bundle> --path-as-root --service keycloak` from a four-file bundle whose contents are listed at the top of `deploy/keycloak/Dockerfile`; that bundle carries its own `railway.json` at its root; after an upload on 2026-09-16 Railway's deployment manifest for the service showed the bundle's values (`dockerfilePath deploy/keycloak/Dockerfile`, `healthcheckPath None`), not the repository's. Linking a second service to the repo needs the Railway dashboard. Upload again after any change |
