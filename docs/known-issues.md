...
| The old demo in MB's workspace comes back on every merge | A second public Sanad appears after each merge to `main`, on MB's trial plan, with a dead sign-in service (its Keycloak is deleted, 404). Stopped twice on 2026-09-16; redeployed both times | `railway down` removes the running container, not the GitHub link, and the CLI cannot unlink a repo. Disconnecting or deleting that service needs MB's Railway dashboard, and deleting a service in someone else's project is not something to do unasked |
| `railway.json` is NOT applied by Railway, and the health gate is set by hand instead | Settled 2026-09-16: the service's settings page showed an empty "+ Healthcheck Path" button while the file in the repository declared one, and two deploys carrying the file reported `healthcheckPath None`. YL then set `/api/v1/health` in the dashboard and the manifest now reports it | Why the file is ignored is unknown (the service was created with `railway add --repo` from the CLI; Railway's documentation says a root config file is picked up automatically). The file stays: it is the reviewable record of what the deployment should be, `tests/unit/test_packaging.py` keeps it honest against both gates, and it costs nothing. A setting that lives only in a dashboard is invisible to everyone reading this repository -- that is the real problem here |
...
| The demo's Keycloak is not linked to GitHub and has a redeploy trap | Changing a variable on the Railway `keycloak` service without `--skip-deploys` redeploys the plain published image, which only prints help and stops; a merge to `main` never updates it | It is uploaded with `railway up <bundle> --path-as-root --service keycloak` from a four-file bundle whose contents are listed at the top of `deploy/keycloak/Dockerfile`; that bundle carries its own `railway.json` at its root, though whether Railway reads it is UNCONFIRMED: the manifest after an upload is consistent with it and equally consistent with the service's `RAILWAY_DOCKERFILE_PATH` variable plus platform defaults. Linking a second service to the repo needs the Railway dashboard. Upload again after any change |
