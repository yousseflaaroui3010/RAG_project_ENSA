...
> AUTOMATED: paste SETUP-PROMPT.md into dev tooling first; it handles
> items 5-8 below and fills VARIABLES.md for you by asking or detecting.

## Install on your machine (once)
1. dev tooling, latest version. Node 22+ and python3 (the hooks use it;
   both ship with WSL2/macOS dev setups).
2. codebase-memory-mcp: in the repo, tell dev tooling
...
5. Copy this kit in, `chmod +x .devtools/hooks/*.sh`, commit.
6. Fill VARIABLES.md targets: ENGINEERING-RULES.md, config.sh, gate.yml, rule globs.
...
8. Restart dev tooling, then verify: /hooks (6 hooks), /memory (global
   rules + ENGINEERING-RULES.md), /agents (11 agents), /mcp (graph server). Test a
...
11. At launch: move .devtools/agents-phase4/o*.md back into .devtools/agents/,
