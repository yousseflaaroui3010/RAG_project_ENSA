...
`intfloat/multilingual-e5-base`: 278M parameters, 1,112 MB at float32. The
published container is capped at 1,024 MB. Both Sync AND Chat load that
model -- Chat too, through `vector_store` line 377's `embed_query` -- so on
that box neither can ever work.

THE FAILURE THIS REPLACES was the worst kind. The platform killed the
process partway through loading the model and restarted it; the browser was
left polling a Sync that no longer existed, showing "Scanning the workspace
folder..." indefinitely, and nothing anywhere said why. Observed in
production on 2026-09-07: container restart 27 seconds after the Sync
started, immediately after `Loading weights`.

WHAT IS DELIBERATELY NOT TESTED HERE: that answering works when the flag is
off. That is every other integration test in this directory. The last test
below checks only that the flag is what stops Sync -- with it off, the run
starts -- because a refusal test that passes for the wrong reason (no
workspace, bad folder, missing schema) would be indistinguishable from a
correct one.
"""
