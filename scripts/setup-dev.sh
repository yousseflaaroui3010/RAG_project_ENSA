...
HOOK_SRC="$ROOT/.devtools/git-hooks/pre-commit"
HOOK_DST="$ROOT/.git/hooks/pre-commit"
if [ ! -f "$HOOK_SRC" ]; then
  bad "hook template missing at .devtools/git-hooks/pre-commit"
...
if command -v codebase-memory-mcp >/dev/null 2>&1; then
  ...
  echo "    the program exists. Without it, the assistant falls back to grep and find,"
