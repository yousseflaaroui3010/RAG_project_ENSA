...
- A convention from the previous major is a wrong answer that compiles.
  T-014 wrote `jsx: preserve` from the Next 15 era; Next 16 rewrote it to
  `react-jsx` on first build. Verify against the version in the lockfile,
  including the parts you are sure about.
- Tools rewrite your config files. Next edits `tsconfig.json` on build.
  Expect it, read the file back after the first run, and keep what the
  tool set unless you know better and can say why.
