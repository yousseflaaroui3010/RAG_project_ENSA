...
<context>
Per task: task card, docs/journal/BUILD-STATE.md, the reviewer's report,
...
3. Re-run the WHOLE gate yourself, not just typecheck: read gate.yml and
   run every step with the same flags. Claims are not checks, and a green
   claim about a step that skipped itself is worth nothing.
